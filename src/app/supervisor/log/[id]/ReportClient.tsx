...
export default function ReportClient({ initialLogs, machineId, logDate, targetLogId }: any) {
  ...
  const ShiftSection = ({ shift, isFirst }: { shift: string, isFirst?: boolean }) => {
    ...
    const dieCode = firstLog?.die_code || lastLog?.die_code || '';
      ...
      <table style={{ width: '100%', borderCollapse: 'collapse', tableLayout: 'fixed', marginBottom: '0', borderTop: isFirst ? 'none' : '2px solid #000' }}>
        <tbody>
          ...
          <tr>
            ...
            <td colSpan={3} style={{ textAlign: 'left', fontWeight: 'bold', padding: '4px 8px', border: '1px solid #000' }}>Die Code: {dieCode}</td>
          ...
          <tr>
            ...
            <td style={{ border: '1px solid #000', textAlign: 'center', padding: '0' }}>
              {firstLog ? <input type="text" value={safeVal(firstLog, 'thickness', 'min')} onChange={e => handleValueChange(firstLog.id, 'thickness', 'min', e.target.value)} style={{ width: '100%', height: '100%', border: 'none', textAlign: 'center', outline: 'none' }} className={Number(safeVal(firstLog, 'thickness', 'min')) < 20.55 || Number(safeVal(firstLog, 'thickness', 'min')) > 20.65 ? 'bg-red-100 text-red-900 font-bold' : ''} /> : ''}
            </td>
            <td style={{ border: '1px solid #000', textAlign: 'center', padding: '0' }}>
              {firstLog ? <input type="text" value={safeVal(firstLog, 'thickness', 'max')} onChange={e => handleValueChange(firstLog.id, 'thickness', 'max', e.target.value)} style={{ width: '100%', height: '100%', border: 'none', textAlign: 'center', outline: 'none' }} className={Number(safeVal(firstLog, 'thickness', 'max')) < 20.55 || Number(safeVal(firstLog, 'thickness', 'max')) > 20.65 ? 'bg-red-100 text-red-900 font-bold' : ''} /> : ''}
            </td>
            <td style={{ border: '1px solid #000', textAlign: 'center', padding: '0' }}>
              {lastLog ? <input type="text" value={safeVal(lastLog, 'thickness', 'min')} onChange={e => handleValueChange(lastLog.id, 'thickness', 'min', e.target.value)} style={{ width: '100%', height: '100%', border: 'none', textAlign: 'center', outline: 'none' }} className={Number(safeVal(lastLog, 'thickness', 'min')) < 20.55 || Number(safeVal(lastLog, 'thickness', 'min')) > 20.65 ? 'bg-red-100 text-red-900 font-bold' : ''} /> : ''}
            </td>
            <td style={{ border: '1px solid #000', textAlign: 'center', padding: '0' }}>
              {lastLog ? <input type="text" value={safeVal(lastLog, 'thickness', 'max')} onChange={e => handleValueChange(lastLog.id, 'thickness', 'max', e.target.value)} style={{ width: '100%', height: '100%', border: 'none', textAlign: 'center', outline: 'none' }} className={Number(safeVal(lastLog, 'thickness', 'max')) < 20.55 || Number(safeVal(lastLog, 'thickness', 'max')) > 20.65 ? 'bg-red-100 text-red-900 font-bold' : ''} /> : ''}
            </td>
          ...
          <tr>
            ...
            <td style={{ border: '1px solid #000', textAlign: 'center' }}>1</td>
            <td style={{ border: '1px solid #000', textAlign: 'center' }}>4Hr</td>
            <td style={{ border: '1px solid #000', textAlign: 'center', padding: '0' }}>{firstLog && <input type="text" value={safeVal(firstLog, 'symmetry', 'min')} onChange={e => handleValueChange(firstLog.id, 'symmetry', 'min', e.target.value)} style={{ width: '100%', border: 'none', textAlign: 'center', outline: 'none' }} className={Number(safeVal(firstLog, 'symmetry', 'min')) > 0.2 ? 'bg-red-100 text-red-900 font-bold' : ''}/>}</td>
            <td style={{ border: '1px solid #000', textAlign: 'center', padding: '0' }}>{firstLog && <input type="text" value={safeVal(firstLog, 'symmetry', 'max')} onChange={e => handleValueChange(firstLog.id, 'symmetry', 'max', e.target.value)} style={{ width: '100%', border: 'none', textAlign: 'center', outline: 'none' }} className={Number(safeVal(firstLog, 'symmetry', 'max')) > 0.2 ? 'bg-red-100 text-red-900 font-bold' : ''}/>}</td>
            <td style={{ border: '1px solid #000', textAlign: 'center', padding: '0' }}>{lastLog && <input type="text" value={safeVal(lastLog, 'symmetry', 'min')} onChange={e => handleValueChange(lastLog.id, 'symmetry', 'min', e.target.value)} style={{ width: '100%', border: 'none', textAlign: 'center', outline: 'none' }} className={Number(safeVal(lastLog, 'symmetry', 'min')) > 0.2 ? 'bg-red-100 text-red-900 font-bold' : ''}/>}</td>
            <td style={{ border: '1px solid #000', textAlign: 'center', padding: '0' }}>{lastLog && <input type="text" value={safeVal(lastLog, 'symmetry', 'max')} onChange={e => handleValueChange(lastLog.id, 'symmetry', 'max', e.target.value)} style={{ width: '100%', border: 'none', textAlign: 'center', outline: 'none' }} className={Number(safeVal(lastLog, 'symmetry', 'max')) > 0.2 ? 'bg-red-100 text-red-900 font-bold' : ''}/>}</td>
            {[...Array(6)].map((_, i) => <td key={i} style={{ border: '1px solid #000' }}></td>)}
          </tr>
          ...
          <tr>
            ...
            <td style={{ border: '1px solid #000', textAlign: 'center', padding: '0' }}>{lastLog && <input type="text" value={safeVal(lastLog, 'parallelism', 'min')} onChange={e => handleValueChange(lastLog.id, 'parallelism', 'min', e.target.value)} style={{ width: '100%', border: 'none', textAlign: 'center', outline: 'none' }} className={Number(safeVal(lastLog, 'parallelism', 'min')) > 0.05 ? 'bg-red-100 text-red-900 font-bold' : ''}/>}</td>
            <td style={{ border: '1px solid #000', textAlign: 'center', padding: '0' }}>{lastLog && <input type="text" value={safeVal(lastLog, 'parallelism', 'max')} onChange={e => handleValueChange(lastLog.id, 'parallelism', 'max', e.target.value)} style={{ width: '100%', border: 'none', textAlign: 'center', outline: 'none' }} className={Number(safeVal(lastLog, 'parallelism', 'max')) > 0.05 ? 'bg-red-100 text-red-900 font-bold' : ''}/>}</td>
            {[...Array(6)].map((_, i) => <td key={i} style={{ border: '1px solid #000' }}></td>)}
          </tr>
          ...
          <tr>
            ...
            <td style={{ border: '1px solid #000', textAlign: 'center' }}>1</td>
            <td style={{ border: '1px solid #000', textAlign: 'center' }}>4Hr</td>
            <td style={{ border: '1px solid #000', textAlign: 'center', padding: '0' }}>{firstLog && <input type="text" value={safeVal(firstLog, 'visual', 'min')} onChange={e => handleValueChange(firstLog.id, 'visual', 'min', e.target.value)} style={{ width: '100%', border: 'none', textAlign: 'center', outline: 'none' }} className={safeVal(firstLog, 'visual', 'min').toUpperCase() === 'NG' ? 'bg-red-100 text-red-900 font-bold' : ''}/>}</td>
            <td style={{ border: '1px solid #000', textAlign: 'center', padding: '0' }}>{firstLog && <input type="text" value={safeVal(firstLog, 'visual', 'max')} onChange={e => handleValueChange(firstLog.id, 'visual', 'max', e.target.value)} style={{ width: '100%', border: 'none', textAlign: 'center', outline: 'none' }} className={safeVal(firstLog, 'visual', 'max').toUpperCase() === 'NG' ? 'bg-red-100 text-red-900 font-bold' : ''}/>}</td>
